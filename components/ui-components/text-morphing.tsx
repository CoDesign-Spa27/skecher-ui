...
export const TextMorph = ({
  ...
  const characters = useMemo(() => getCharacterTokens(children, uniqueId), [children, uniqueId]);
  const animate =
    variants === defaultVariants
      ? {
          opacity: 1,
  ...
  return (
    <Component
      aria-label={children}
      className={cn("relative isolate overflow-hidden whitespace-pre leading-none", className)}
      style={style}
    >
...
export const MorphingText = ({ texts, className, interval = 2000 }: MorphingTextProps) => {
